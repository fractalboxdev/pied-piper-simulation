...
- `pnpm slack:user-tokens` — collect a user token (xoxp) for each persona's *real* sandbox account via a paste-the-code OAuth flow (`scripts/slack/collect-user-tokens.ts`); tokens are appended to `.env` as `SLACK_USER_TOKEN_<SLUG>`. Requires `SLACK_CLIENT_ID`/`SLACK_CLIENT_SECRET`/`SLACK_OAUTH_REDIRECT_URL` and the accounts to exist (invited as `pied-piper-sim-<slug>@<domain>`; sandbox cap is 8 members incl. admin). See the script header.
...
- `pnpm generate:invoices` — render three mock Anthropic (Claude API) vendor invoices payable by Pied Piper into `fixtures/invoices/*.pdf` via `scripts/invoices/generate-anthropic-invoices.ts` (pdf-lib; line-item data lives in `scripts/fixtures/anthropic-invoices.ts`). Fully deterministic (explicit line-item data; PDF metadata dates pinned to each invoice's issue date — re-runs are byte-identical); each PDF is marked SPECIMEN and carries `timeline_id` + `sim_event_id` in the footer. No env vars needed.
- `pnpm seed:stripe` — seed usage-based **revenue by client** into a Stripe test account via `scripts/stripe/seed-fixtures.ts`: one test clock + customer + metered subscription per fixture company (flat tier base fee + per-GB compression price on a shared billing meter), compression usage replayed as meter events while the clock advances in lockstep with `sim_time`, final advance triggers the month's invoices. Requires `STRIPE_SECRET_KEY` (test-mode only, refuses live keys); `--replace` deletes this timeline's test clocks and reseeds. Pricing constants are shared with `scripts/fixtures/economics.ts`.
- `pnpm report:economics` — print the per-client unit-economics join (`scripts/fixtures/economics.ts`): usage-based revenue (base fee + metered GB) × the window month's Anthropic invoice allocated by tier-weighted compressed-bytes share → **Claude cost per KB of compression per client**, $/GB, and gross margin. Pure projection over the fixture dataset; no env vars.
